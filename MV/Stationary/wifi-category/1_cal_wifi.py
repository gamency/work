...
filter_str = "occur_date >= '%s' and occur_date < '%s' and %s is not null" % (dt_before120_str, dt_str, "bssid")
data = spark.read.parquet("lbs/raw_data").filter(filter_str)
data = prepare_data(data)
feature = compute_feature(data, "bssid", "")
feature.repartition(200).write.parquet("lbs/model/wifi_sub_feature", mode="overwrite")