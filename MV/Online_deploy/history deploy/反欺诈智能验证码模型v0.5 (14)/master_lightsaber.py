...
def shortage_of_s_data_tmp(data, correlation_s):
    #correlation = cdll.LoadLibrary(os.getcwd() + '/tmp_m1.so')
    tulasion = np.mean(data)
    ticks = time.time()
    coeffeciency = correlation_s.test(c_double(ticks), c_float(tulasion))
    return coeffeciency
...
def lightsaber(input_dataframe, input_dataframe_s, model, verify_inf, correlation):
    
    #print("before", colon_names)
    #if len(input_dataframe) <= 4:
    ...
    hit_bar_center_flag = colon_military.loc[0, "hbc"]
    
    #print(colon_military.describe())
    
    #from sklearn import preprocessing 
    #for f in colon_military.columns: 
    #    if colon_military[f].dtype=='object': 
    #        lbl = preprocessing.LabelEncoder() 
    #        lbl.fit(list(colon_military[f].values)) 
    #        colon_military[f] = lbl.transform(list(colon_military[f].values))
    #        print("hithehe")
        #print("okkkk")
    ...
    #colon_military = colon_military.astype(float)
    colon_military = np.array(colon_military)
    colon_military = colon_military.astype(float)
    # xgboost 
    dpredict = xgb.DMatrix(colon_military)
    jedis_result = model.predict(dpredict)
    ...
    if verify_inf['terminal'] !=0:
        #print("it's mobile")
        coeffeciency = shortage_of_s_data_tmp(input_dataframe_s, correlation)
        if hit_bar_center_flag == 0.5 or coeffeciency:
        ...
        #    return obi_won(0, 0)
            ...
            #print("jedis result1 is",jedis_result1)

            jedis_result = jedis_result * 0.3 + jedis_result1 * 0.7 * coeffeciency
    else:
